...
const ProfilePage = ({ onBack, credits, onNav }) => (
  <Shell title="Profile" onBack={onBack}>
    <div style={{background:`linear-gradient(145deg,${$.ac}10,${$.ac2}06)`,borderRadius:22,padding:30,textAlign:"center",border:`1px solid ${$.ac}15`,marginBottom:20}}>
      ...
      <div style={{fontSize:13,color:$.t2,marginTop:4}}>Level {USER.level} · ActiveSG Member</div>
      <div style={{margin:"12px auto 0",display:"inline-flex",alignItems:"center",gap:8,background:`${$.ac}15`,border:`1px solid ${$.ac}30`,borderRadius:20,padding:"6px 16px"}}>
        <span style={{fontSize:16}}>🏅</span>
        <span style={{fontSize:15,fontWeight:800,color:$.ac}}>{credits}</span>
        <span style={{fontSize:12,color:$.t2}}>ActiveSG Credits</span>
      </div>
      <div style={{margin:"14px auto 0",width:"55%"}}><div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:$.t3,marginBottom:5}}><span>XP</span><span>{USER.xp.toLocaleString()}/{USER.xpMax.toLocaleString()}</span></div><div style={{height:8,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}><div style={{width:`${(USER.xp/USER.xpMax)*100}%`,height:"100%",background:`linear-gradient(90deg,${$.ac},${$.ac2})`,borderRadius:4,boxShadow:`0 0 10px ${$.ac}40`}}/></div></div>
    </div>
    ...
    {[
      { label: "Account Settings", action: null },
      { label: "Notifications", action: null },
      { label: "Privacy", action: null },
      { label: "🔐 Equipment Locker", action: () => onNav("locker") },
      { label: "Help & Support", action: null },
    ].map(item => (
      <div key={item.label} onClick={item.action || undefined} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 0",borderBottom:`1px solid ${$.bd}`,cursor:"pointer"}}>
